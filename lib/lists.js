...
function List(service, options, methods)
{
    Watched.call(this)
...
function DumbList (service, options, methods)
{
    List.call(this, service, options, methods)
}
...
DumbList.prototype.start = function ()
{
    this.service('find', this.options, function (err, records)
    {
        ...
        else
        {
            ...
            {
                this.records[i] = new Record(records[i], this.methods)
            }
        }
        computation.async()
    }.bind(this))
...
{
    var record = new Record(fields, this.methods)
...
function WatchList (service, options, methods)
{
    List.call(this, service, options, methods)
    this.channel = null
    this.started = false
    ...
    this.service('find', this.options, function (err, channel)
    {
        ...
        else
        {
            ...
            channel.on('initial', function (records)
            {
                ...
                for (var i=0; i<records.length; i++)
                {
                    var record = new Record(records[i], this.methods)
            ...
            channel.on('insert', function (fields)
            {
                ...
                {
                    record = new Record(fields, this.methods)
                    this.records.push(record)
                    this.records_by_id[fields.id] = record
                }
...
function SyncListRecord (list, fields, methods)
{
    WritableRecord.call(this, fields, methods)
    this.list = list
    this.syncdoc = null
}
SyncListRecord.prototype = Object.create(WritableRecord.prototype)
...
function SyncList (service, fields, methods)
{
    WatchList.call(this, service, fields, methods)
...
{
    var record = new SyncListRecord(this, records[i], this.methods)
    this.records.push(record)
    this.records_by_id[record.fields.id] = record
}
    ...
    this.service('find', this.options, function (err, channel)
    {
        ...
        else
        {
            ...
            channel.on('insert', function (fields)
            {
                ...
                {
                    record = new SyncListRecord(this, fields, this.methods)
                    record.syncdoc = new SyncDocument(fields)
                    this.records_by_id[record.fields.id] = record
                    this.records.push(record)
                }
...
SyncList.prototype.push = function (record)
{
    ...
    if (record.syncdoc.edits.length > 0)
    {
        this.channel.emit('pull', record.fields.id, record.syncdoc.edits)
    }
}